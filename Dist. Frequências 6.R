Consumo = c(32, 40, 22, 11, 34, 40, 16, 26, 23, 31, 27, 10, 38, 17, 13,
            45, 25, 10, 18, 23, 35, 22, 30, 14, 18, 20, 13, 24, 35, 29,
            33, 48, 20, 12, 31, 39, 17, 58, 19, 16, 12, 21, 15, 12, 20,
            51, 12, 19, 15, 41, 29, 25, 13, 23, 32, 14, 27, 43, 37, 21,
            28, 37, 26, 44, 11, 53, 38, 46, 17, 36, 28, 49, 56, 19, 11)


frequencia_simples = table (Consumo)
TabelaC <- data.frame(frequencia_simples)

frequencia_relativa_simples = frequencia_simples/sum(frequencia_simples)*100
TabelaC$frporcentual = frequencia_relativa_simples #acrescenta coluna
frequencia_acumulada = cumsum(frequencia_simples)
TabelaC$FAC = frequencia_acumulada
frequencia_relativa_acumulada = frequencia_acumulada/sum(frequencia_simples)*100
TabelaC$FACporcentual = frequencia_relativa_acumulada
plot(TabelaC$Consumo, TabelaC$frporcentual)
ggplot(TabelaC, aes (x = Consumo, y = frequencia_relativa_simples)) + geom_histogram(stat = "identity")
